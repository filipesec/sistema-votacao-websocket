...
function inicializarAplicacao() {
    ...
    if (!usuarioId) {
        console.log('Sem cookie, solicitando...');
        fetch('/obter-usuario-id')
            .then(response => response.json())
            .then(data => {
                console.log('Cookie recebido, recarregando...');
                location.reload();
            })
            .catch(error => {
    ...
    console.log('Cookie encontrado:', usuarioId);
...
function inicializarGrafico() {
    ...
    const votos = JSON.parse(localStorage.getItem('votos')) || Array(13).fill(0);
...
// Inicia a aplicacao quando a pagina carrega
